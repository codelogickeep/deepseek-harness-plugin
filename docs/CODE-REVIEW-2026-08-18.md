...
> 关键结论（时区 bug）已独立复现验证。
> 状态：**暂存待修**。本文件是修复清单，不是修复记录。

---

## 🔴 严重（4 个功能 bug，建议优先修复）

### 1. cron 定时任务「时区从未生效」—— 触发时刻静默偏差 8 小时 ⚠️最严重
...
| 9 | `src/sessions.js:69-77` | `_save` 非原子写（直接 writeFileSync 覆盖），崩溃可能损坏 mapping JSON | 先写临时文件再 rename |
...
- **覆盖缺口**：时区、DST、跨会话去重、并发回复、lastFired 回拨等关键分支无负向测试。
