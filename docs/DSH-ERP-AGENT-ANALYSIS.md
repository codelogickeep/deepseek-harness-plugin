...
### 特性 7：subagent 多 provider —— 多 Agent 协同 / 模型自由编排
- **是什么**：subagent seam 支持多种 provider，**per-agent 模型指定**——主 agent 用强模型、子 agent 用便宜模型，各司其职。我们的 `flash-worker` preset 就是这种"两级协同"。
- **准确口径（不夸大）**：**"多 agent 配不同模型"不是 dsh 独有**——Claude Code / Codex 也能（CC 限 `haiku/sonnet/opus`、Codex 限 `gpt-5.4` 系列，均为自家档位）；**openclaw 也能，而且和 dsh 一样是跨厂商任意模型**（开源多模型网关，`agents.list[].subagents.model` + `agents.defaults.models["provider/model"]`）。真正把"多模型"钉死在自家生态里的只有 CC/Codex。对 ERP 的意义：主 agent 用强模型做规划/判断，子 agent 用便宜模型做批量/执行，按任务性价比配模型——dsh 只是众多能做到这一点的开源 harness 之一，选型时综合考虑。
...
#### 5.8.3c 能力矩阵对"按领域分 preset"有什么用

> 追问："能力矩阵在按领域划分 preset 时，具体起什么作用？" 答案：**让"领域"从"一组工具"升级为"一个完整的岗位"**——如果 preset 只能配模型+工具，领域预设就只是"换了工具箱的同一个工人"；有了能力矩阵，每个领域 preset 是**工作方式、思考深度、团队构成都不同**的独立 Agent。逐层看对 ERP 的作用：

| 能力层 | 领域 preset 里怎么用 | 对 ERP 业务的具体作用 |
| --- | --- | --- |
| **① 身份/提示词** | 每领域自己的 persona + 指令 | 销售 preset 是"资深销售分析师"（主动问口径/报增长率）、库存 preset 是"仓储管理员"（关心安全库存/缺货预警）、财务 preset 是"合规员"（先讲凭证/审批）——**同一问题，三个领域三种专业视角** |
| **② 工具权限** | 每领域挂自己的工具集 | 销售挂 `query_sales`/`price`，库存挂 `query_stock`/`reorder_alert`，财务挂 `query_finance`/`approval_flow`——**领域 Agent 天生摸不到别领域工具**（比同一工具集+restrict 更彻底，根本不在 schema）|
| **③ 能力开关** | 每领域开/关/配不同 planning、delegation | 销售分析 preset 开 `planning`（先出计划再动手）、关 `delegation`（单兵够用）＝"深思熟虑的分析师"；erp-dev preset 开 `planning`+`delegation`+配 compaction＝"会带团队的大工程 Agent"；查单 preset 全关＝"快速应答的接线员"——**同样一套内核，不同领域 = 不同思考深度+协作规模** |
| **④ 协作模式** | 每领域声明自己的"团队" | 复杂领域（研发）配 `tool-subagent-flash`（便宜模型批量）+ 可选 codex；简单领域（查单）不配 subagent——**每个领域的成本结构、并行能力不同** |
| **⑤ 作用域隔离** | 每领域独立实例 | 销售会话的 persona/工具/plan 状态不泄漏到库存会话；多客户多领域同进程跑互不污染——**"按领域服务化"能成立的基础** |

**一句话**：能力矩阵让"领域预设"成为**一份完整的岗位说明书（职位描述 + 组织架构 + 行为准则）**，而不只是"换工具箱"。对 ERP 的价值 = 每个业务域一个真正独立、专业、可配协作规模与成本的 Agent——这也是"多领域服务化（5.8）"值得做的核心原因。
