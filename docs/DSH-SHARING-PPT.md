...
今天分享 DeepSeek Harness（DSH），一个 8 月 13 号刚开源的 Agent 框架。我花了两周，从"用它的用户"变成了"给它写插件的人"——期间还把它搞崩过三次。今天这十几分钟，我会讲清楚三件事：**它是什么（设计态）、它站在哪儿（定位）、以及用它做真实业务（ERP 场景）能到什么程度**。
...
**这跟"按领域分 preset"结合，价值就出来了**：如果 preset 只能配模型+工具，领域预设就只是"换了工具箱的同一个工人"；有了能力矩阵，**每个领域 preset 是一个工作方式不同的独立 Agent**——销售 preset 是"资深分析师"（主动问口径、报增长率）、库存 preset 是"仓储管理员"（关心安全库存、缺货预警）、财务 preset 是"合规员"（先讲凭证/审批）；复杂领域（如研发）开 planning+delegation、配便宜子 agent 批量干活，简单领域（如查单）全关、快速应答；而且各领域会话 isolate 隔离、互不串味。所以"按领域服务化"不是"给不同领域换工具包"，而是**为每个业务域定义一份完整的岗位说明书（职位描述 + 组织架构 + 行为准则）**。
