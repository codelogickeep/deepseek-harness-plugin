...
- id: delegation
  ...
  config:
    ...
    # 「pro 指挥、flash 执行」：flash_agent 把委派任务交给 flash 模型子 agent。
    # 主 agent（pro）用它派发具体编码/执行任务，拿回结果后 review；保留全部工具集，不 toolFilter。
    # provider/model 由 install-flash-preset 脚手架在安装时注入占位符。
    - id: tool-subagent-flash
      name: '@deepseek-ai/dsh-tool-subagent'
      config:
        provider: spawn
        toolName: flash_agent
        backgroundMode: continuable
        agentOptions:
