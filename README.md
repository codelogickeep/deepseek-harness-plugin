...
| **项目文件树** | 右上角**最右**「⋮☰」按钮，右侧浅色融入式文件树（对齐左侧栏风格）：递归目录 + git 状态徽标；**可拖拽调宽**（220-520）；**双击文件在 IDE 中打开**；头部**路径显示+复制**（`./` 开头，最多三行，选中项高亮）；底部 git 汇总（分支/↑领先/N 处变更/最近提交）；**fs.watch+SSE 实时刷新**（文件增删/git 提交即时更新，非定时轮询） |
